...
import { contract } from "@rapidkey/contracts/index";
import { writeFileSync, mkdirSync } from "fs";
import { EndpointMetadata, PermissionId } from "@rapidkey/contracts/util/api";
...
import {
  ...
  RateLimiterId,
  Window,
} from "@rapidkey/contracts/rate-limit/index";
  ...
  const openApiDocument = generateOpenApi(
    contract,
    {
      ...
      info: {
        title: "RapidKey API",
        description:
          "Documentation for the endpoints provided by the RapidKey API server.\n\nNote that authentication is performed with the Authorization HTTP header in the format `Authorization: ApeKey YOUR_APE_KEY`\n\nThere is a rate limit of `30 requests per minute` across all endpoints with some endpoints being more strict. Rate limit rates are shared across all ape keys.",
        version: "2.0.0",
        termsOfService: "https://rapidkey.com/terms-of-service",
        contact: {
          name: "Support",
          email: "support@rapidkey.com",
        },
        "x-logo": {
          url: "https://rapidkey.com/images/mtfulllogo.png",
        },
        license: {
          name: "GPL-3.0",
