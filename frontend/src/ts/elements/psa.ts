...
import { IdSchema } from "@rapidkey/schemas/util";
import { tryCatch } from "@rapidkey/util/trycatch";
import { isSafeNumber } from "@rapidkey/util/numbers";
  ...
  if (response.status === 500) {
    if (isDevEnvironment()) {
      ...
      const { data: instatus, error } = await tryCatch(
        fetch("https://rapidkey.instatus.com/summary.json")
      );
      ...
      if (
        ...
        Notifications.addPSA(
          "Looks like the server is experiencing unexpected down time.<br>Check the <a target= '_blank' href='https://rapidkey.instatus.com/'>status page</a> for more information.",
          -1,
          "exclamation-triangle",
          false,
          undefined,
    ...
    Notifications.addPSA(
      "Server is currently under maintenance. <a target= '_blank' href='https://rapidkey.instatus.com/'>Check the status page</a> for more info.",
      -1,
      "bullhorn",
      true,
      undefined,
