...
import { ChartData } from "@rapidkey/schemas/results";
import { Mode, Mode2, Mode2Custom } from "@rapidkey/schemas/shared";
import { ResultFiltersGroupItem } from "@rapidkey/schemas/users";
import { findLineByLeastSquares } from "../utils/numbers";
import defaultResultFilters from "../constants/default-result-filters";
import { SnapshotResult } from "../constants/default-snapshot";
import Ape from "../ape";
import { AccountChart } from "@rapidkey/schemas/configs";
  ...
  DB.getSnapshot()?.results?.forEach((result) => {
    ...
    try {
      ...
      if (
        ...
        ResultFilters.getFilter("language", "english")
      ) {
        langFilter = true;
      }
      if (!langFilter) {
