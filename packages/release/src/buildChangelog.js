...
function getPrLink(pr) {
  ...
  return `[#${prNum}](https://github.com/rapidkeygame/rapidkey/pull/${prNum})`;
}

function getCommitLink(hash, longHash) {
  return `[${hash}](https://github.com/rapidkeygame/rapidkey/commit/${longHash})`;
}
...
function convertStringToLog(logString) {
  ...
  for (let line of logString) {
    ...
    const [, type, scope, message, username, pr] = title.split(
      /^(\w+)(?:\(([^)]+)\))?:\s+(.+?)(?:\s*\((@[^)]+)\))?(?:\s+\((#[^)]+)\))?$/
    );
